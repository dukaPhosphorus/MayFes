...
class KeyMetadatas(Metadata):
    ...
    def evaluate(self, chorale):
        ...
        for part in chorale.parts:
            chorale_with_measures.append(part.makeMeasures())#なんか時間のoffsetが追加される
        #########このタイミングでchorale_with_measures.show()をすると元の楽譜と同じのが表示される
        """
        >>> sSrc.show('text')
        ...
        res = ka.run()
        """resは[<music21.key.Key of G major>, <music21.key.Key of a minor>, <music21.key.Key of a minor>, <music21.key.Key of D major>, <music21.key.Key of D major>,,,,,,,, <music21.key.Key of G major>]"""
    ...
    def generate(self, length):
        return np.full((length,), self.get_index(2))
